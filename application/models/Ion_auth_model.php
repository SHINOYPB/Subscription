...
class Ion_auth_model extends CI_Model
{
    ...
    public function delete_grp($table_name,$id) {
        if (!is_array($id)) {
            $id = array('id' => $id);
        }
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description, '.$this->tables['groups'].'.permissions')
...
// 		$where[];
		$gid= 0;
		$this->response = $this->db->where('groups.status',$gid)->get($this->tables['groups']);
